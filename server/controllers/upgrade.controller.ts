...
export const upgrade_get = async (
    req: MyRequest,
    res: Response,
    next: NextFunction
  ) => {
  ...
  export const upgrade_post = async (
    req: MyRequest,
    res: Response,
    next: NextFunction
  ) => {
    try {
        ...
        if(nft.level == 0){
            ...
            if(user.token_amount >= money){
                ...
                const withdraw = await erc20Contract.methods.transfer(process.env.SERVER_ADDRESS,money)
                ...
                if(!isSuccess){
                    ...
                    const approve = await erc721Contract.methods.approve(process.env.SERVER_ADDRESS,nft.token_id)
                    .send({from:user.address,gas:500000});
                    const transfer = await erc721Contract.methods.transferNFT(user.address,process.env.SERVER_ADDRESS,nft.token_id)
                    .send({from:user.address,gas:500000});
                    const result = await erc721Contract.methods.mintNFT(user.address,upgradeCard.img_url,upgradeCard.player,upgradeCard.season,0)
                    ...
                    if(result){
                        const mintedNft = await db.Nft.create({
                            ...
                            states:"강제할당",
                            level:upgradeCard.level,
                            price:upgradeCard.price,
