...
export const detail_get = async (
  ...
  try {
    ...
    // 조회수 증가
    const result = await db.Post.post.increment("views", { by: 1 });
...
// 글 수정하기
export const editPost_get = async (
  req: MyRequest,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    res.status(200).send({ data: { content, title } });
  } catch (error) {
    console.log(error);
  }
};

export const editPost_post = async (
  req: MyRequest,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    const result = await db.Post.update({ title, content }, { where: { id } });
    // 4. 프론트에 성공했다 알려주기
...
export const deletePost_delete = async (
  req: MyRequest,
  res: Response,
  next: NextFunction
) => {
...
// 댓글 작성
export const comment_post = async (
  req: MyRequest,
  res: Response,
  next: NextFunction
) => {
...
// 댓글 좋아요 / 싫어요
export const likeComment_post = async (
  req: MyRequest,
  res: Response,
  next: NextFunction
) => {
